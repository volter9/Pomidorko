...
return 0>t?(this.view.render(0),this.stop()):void this.view.render(t/1e3)},s.prototype.pause=function(){clearInterval(this.timer),this.timer=null,this.remained=this.endTime-Date.now(),this.startTime=this.endTime=null},s.prototype.stop=function(){this.remained=this.endTime=this.startTime=null,clearInterval(this.timer),this.timer=null,this.emit("stop")},s.prototype.isRunning=function(){return Boolean(this.timer)},s.prototype.isTimeRemained=function(){return null!==this.remained},e.exports=s},{"./helpers/events":2}],14:[function(t,e,i){var n=t("../mvc/view"),s=n.extend({initialize:function(){this.data.button.addEventListener("click",this.toggleView.bind(this)),this.bind(".close","click",this.close)},show:function(){this.node.classList.remove("hidden"),this.toggle(this.node.classList.contains("hidden"))},close:function(){this.node.classList.add("hidden"),this.toggle(this.node.classList.contains("hidden"))},toggleView:function(){this.node.classList.toggle("hidden"),this.toggle(this.node.classList.contains("hidden"))},toggle:function(t){this.node.classList.toggle("about-appear",!t),this.node.classList.toggle("about-disappear",t)}})
e.exports=s},{"../mvc/view":12}],15:[function(t,e,i){var n=t("../mvc/view"),s=t("./goals"),o=t("./about"),r=t("./timer"),a=t("./settings"),h=n.extend({initialize:function(){var t=this.node,e=this.data.goals,i=new Audio("assets/bell.mp3")
e.on("change",function(){t.classList.toggle("resting",e.get("recess"))}),this.timer=new r(this.find("#timer"),{settings:this.data.settings,recess:this.find(".pa-recess span"),goals:this.data.goals}),this.goals=new s(this.find("#goals"),this.data),this.about=new o(this.find(".about"),{button:this.find("#about")}),this.settings=new a(this.find(".pa-settings"),{button:this.find(".pa-settings-button"),settings:this.data.settings}),this.timer.timer.on("start",function(){var t=!!e.get("recess")
document.title=(t?"Перерыв":"Работа")+" — помидорковый таймер",document.querySelector("link[rel=icon]").href=t?"assets/img/break.ico":"assets/img/work.ico",document.querySelector(".pa-recess span").innerHTML=t?"Пропустить перерыв":"Пропустить помидорку"}),this.timer.timer.on("stop",function(){i.play()})}})
...
if(this.find(".current").innerHTML=t.get("current"),this.find(".total").innerHTML=e.get("total"),t.get("recess")===!0){var i=s[o(1,s.length)-1],n=this.find(".status")
