...
module.exports = View.extend({
    ...
    setupEvents: function () {
        var self  = this,
            ...
            sound = new Audio('assets/sounds/bell.mp3');
        ...
        timer.on('stop', this.stop.bind(this));
    },
    ...
    update: function () {
        var recess = this.data.goals.get('recess'),
            icon   = recess ? 'assets/img/break.ico' : 'assets/img/work.ico';
        
        this.template = lang.get(recess ? 'recess' : 'work')
            + ' — ' + lang.get('title', 'помидорковый таймер');
        
        if (!this.timer.timer.isRunning()) {
            this.changeTitle();
        }
    ...
    changeTitle: function (title) {
        ...
        document.title = title + this.template;
    },
    ...
    stop: function () {
        ...
        if (this.data.settings.get('notifications') === true) {
            notify.notify(lang.get(recess ? 'recess' : 'pomidorko'), icon);
        }
    }
});